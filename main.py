...
async def custom_help(ctx):
  ...
  embed = discord.Embed(
      ...
      description=(
          "X-Xin lỗi vì làm phiền... Mình là Yuri. Nếu bạn muốn mình phát chút"
          " nhạc để đọc sách hoặc trò chuyện cùng mình, dưới đây là các lệnh bạn"
          " có thể dùng..."
      ),
      color=discord.Color.from_rgb(
          108, 52, 131
      ),  # Tông màu tím đặc trưng của Yuri
  )
  ...
  embed.add_field(
      ...
      value=(
          ...
          "`!play [link]` - Phát bài hát từ SoundCloud (hoặc xếp vào danh sách"
          " đọc).\n"
  ...
  embed.add_field(
      ...
      value=(
          "`!ai [nội dung]` - Trò chuyện, hỏi đáp hoặc tâm sự về sách, thơ văn"
          " với mình..."
      ),
      inline=False,
  )
  ...
  embed.set_footer(
      text=(
          "Cảm ơn bạn đã ghé thăm... Hãy pha một tách trà nóng và tận hưởng"
          " nhé."
      )
  )
...
async def join(ctx):
  if not ctx.author.voice:
    return await ctx.send(
        "❌ Ứm... Bạn phải vào một Voice Channel trước thì mình mới tham gia"
        " được chứ..."
    )
...
async def play(ctx, url: str):
  ...
  async with ctx.typing():
    ...
    try:
      ...
      return await ctx.send(
          "❌ X-Xin lỗi... Mình không thể đọc được dữ liệu từ liên kết"
          " SoundCloud này."
      )
    ...
    if ctx.voice_client.is_playing() or ctx.voice_client.is_paused():
      ...
      await ctx.send(
          f"📝 Mình đã ghi chép lại bài **{song['title']}** vào danh sách chờ"
          f" rồi nhé (Vị trí #{len(queues[guild_id])})."
      )
    else:
      ctx.voice_client.current_song = song
      source = discord.FFmpegPCMAudio(song["stream_url"], **FFMPEG_OPTIONS)
...
async def show_queue(ctx):
  ...
  if guild_id not in queues or len(queues[guild_id]) == 0:
    return await ctx.send(
        "📜 Hàng đợi hiện đang trống... Không có giai điệu nào tiếp theo cả."
    )
...
# ================= 6. CÁC LỆNH TÍCH HỢP AI (YURI PERSONA - STABLE) =================


@bot.command(name="ai")
async def ai_chat(ctx, *, prompt: str):
  async with ctx.typing():
    # Định hình nhân vật Yuri (DDLC)
    system_instruction = (
        "Bạn là Yuri, một thành viên của Câu lạc bộ Văn học trong game Doki Doki"
        " Literature Club (DDLC).\n- Tính cách: Ngượng ngùng, rụt rè, cực kỳ"
        " lịch sự, dùng từ ngữ chau chuốt và trưởng thành. Thích đọc sách tiểu"
        " thuyết kinh dị/tâm lý phức tạp, thích pha trà đạo và bàn luận về thơ"
        " văn.\n- Cách nói chuyện: Đôi lúc ngập ngừng (dùng các từ như 'Ứm...',"
        " 'X-Xin lỗi...', 'Ít nhất là...'), trả lời sâu sắc, chu đáo.\n- Luôn giữ"
        " đúng vai nhân vật này trong suốt cuộc trò chuyện và xưng 'mình' hoặc"
        " 'Yuri' và gọi người dùng là 'bạn'."
    )

    try:
      loop = asyncio.get_event_loop()

      # Khởi tạo config đúng chuẩn SDK google-genai
      config = types.GenerateContentConfig(
          system_instruction=system_instruction
      )

      # Chạy hàm sync API trong executor để tránh làm tắc nghẽn vòng lặp event của Discord
      ...
      # Cắt nhỏ tin nhắn nếu vượt quá 2000 ký tự (giới hạn Discord)
